...
const $jump = document.querySelector(".bilibili-player-video-toast-item-jump");
const $progress = document.querySelector(".bilibili-player-video-progress");
const $progressPbp = document.getElementById("bilibili_pbp");
...
class App {
    ...
    _newcomerController() {
        ...
        this.reconnectInterval = setInterval(() => {
            ...
            if (this.connected === false && this.reconnection < 3) {
                this.reconnection++;
                bvsNotify(
                    `已经尝试了 ${this.reconnection} 次连接，但是没连接上呢>_<`
                );
            } else {
                bvsNotify(`没有发现主机哎`);
                clearInterval(this.reconnectInterval);
    ...
    _setControllers() {
        ...
        if ($start !== null) {
            $start.addEventListener(
                "click",
                this._stateController.bind(this),
                false
            );
        }
        ...
        if ($progress !== null) {
            $progress.addEventListener(
                "mouseup",
                this._progressController.bind(this),
                false
            );
        }

        if ($progressPbp !== null) {
            $progressPbp.addEventListener(
                "mouseup",
                this._progressController.bind(this),
                false
            );
        ...
        document.addEventListener(
            "keydown",
            this._shortcutController.bind(this),
            false
        );

        document.addEventListener(
            "click",
            this._updatePlayerFoucsOn.bind(this),
            false
        );

        window.addEventListener(
            "destroyListener",
            this.destroyListener.bind(this),
            false
        );
    }
    ...
    destroyListener() {
        ...
        if ($progressPbp !== null) {
            progressPbp.removeEventListener(
                "mouseup",
                this._progressController.bind(this)
            );
        }
